...
// إنشاء جدول إعدادات التكتات مع حفظ رسالة الترحيب
db.exec(`
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (commandName === 'تكت-اعضاء') {
            ...
            try {
                ...
                const stmt = db.prepare('INSERT OR REPLACE INTO ticket_system1 (guild_id, support_role_id, ticket_counter, category_id, welcome_message) VALUES (?, COALESCE((SELECT support_role_id FROM ticket_system1 WHERE guild_id = ?), NULL), COALESCE((SELECT ticket_counter FROM ticket_system1 WHERE guild_id = ?), 0), ?, ?)');
                stmt.run(interaction.guildId, interaction.guildId, interaction.guildId, category.id, welcomeMsg);

                await interaction.reply({ content: `تــم إرســال نــظــام الــتــكــتــات بــنــجــاح!`, ephemeral: true });
            } catch (error) {
                console.error(error);
        ...
        if (commandName === 'رتبة-دعم-اعضاء') {
            ...
            try {
                const stmt = db.prepare('INSERT OR REPLACE INTO ticket_system1 (guild_id, support_role_id, ticket_counter, category_id, welcome_message) VALUES (?, ?, COALESCE((SELECT ticket_counter FROM ticket_system1 WHERE guild_id = ?), 0), COALESCE((SELECT category_id FROM ticket_system1 WHERE guild_id = ?), NULL), COALESCE((SELECT welcome_message FROM ticket_system1 WHERE guild_id = ?), NULL))');
                stmt.run(interaction.guildId, role.id, interaction.guildId, interaction.guildId, interaction.guildId);
        ...
        if (commandName === 'تكت-ثاني') {
            ...
            try {
                ...
                // حفظ رسالة الترحيب والكاتيجوري
                const stmt = db.prepare('INSERT OR REPLACE INTO ticket_system2 (guild_id, support_role_id, ticket_counter, category_id, welcome_message) VALUES (?, COALESCE((SELECT support_role_id FROM ticket_system2 WHERE guild_id = ?), NULL), COALESCE((SELECT ticket_counter FROM ticket_system2 WHERE guild_id = ?), 0), ?, ?)');
                stmt.run(interaction.guildId, interaction.guildId, interaction.guildId, category.id, welcomeMsg);
        ...
        if (commandName === 'رتبة-دعم-ثاني') {
            ...
            try {
                const stmt = db.prepare('INSERT OR REPLACE INTO ticket_system2 (guild_id, support_role_id, ticket_counter, category_id, welcome_message) VALUES (?, ?, COALESCE((SELECT ticket_counter FROM ticket_system2 WHERE guild_id = ?), 0), COALESCE((SELECT category_id FROM ticket_system2 WHERE guild_id = ?), NULL), COALESCE((SELECT welcome_message FROM ticket_system2 WHERE guild_id = ?), NULL))');
                stmt.run(interaction.guildId, role.id, interaction.guildId, interaction.guildId, interaction.guildId);
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId === 'open_ticket_system1') {
            ...
            try {
                ...
                const welcomeMessage = settings.welcome_message || `مــرحــبــاً <@${interaction.user.id}>!\nفــريــق الــدعــم ســيــرد عــلــيــك قــريــبــاً`;
                ...
                await ticketChannel.send({ content: `<@&${settings.support_role_id}> ${welcomeMessage}`, components: [row] });
